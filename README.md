...
本项目当前通过Git仓库提供源码，使用`git clone`获取，不提供独立的软件压缩包、数字签名文件或SHA-256校验文件。获取源码后，可检查Git对象完整性、源码提交版本及工作区状态。
...
在编译或接入优化源码之前，检查工作区和暂存区是否有修改或额外文件：
...
> **说明：**
>
>Git对象完整性检查不能单独证明发布者身份，也不等同于数字签名验证。源码地址和用于核对的目标提交编号仍需通过可信渠道确认。

### 3. 结果判定

| 校验结果 | 判定及处理 |
| --- | --- |
| Git对象检查通过、提交编号与可信目标一致，且工作区检查无输出 | 源码完整性及版本检查通过，可继续使用。 |
| Git对象损坏或缺失 | 不应继续使用该副本，应重新克隆仓库。 |
| 提交编号与目标不一致 | 确认所需源码版本后再继续。 |
| 工作区检查有输出 | 核查本地修改、未跟踪文件和被忽略文件，确认后再继续。 |
| 未获取可信的目标提交编号 | 只能检查本地对象完整性，无法确认是否为预期交付版本。 |

### 4. 异常处理

校验失败时，请保留需要的本地修改，在新目录中从可信仓库地址重新克隆，并重新执行上述检查。如问题仍未解决，请向仓库维护者反馈错误信息及对应提交编号。
